...
- استخدم نموذج توليد صور ونموذج برمجة داخل المحادثة المحلية نفسها.
- ادخل وضع الصور عندما تحتاج إلى أصول بصرية أو أيقونات أو ملصقات أو مراجع تصميم، ثم عد إلى نموذج البرمجة لتطبيقها في المشروع.
- تبقى الصورة المولدة والprompt وتفاصيل المزود والسجلات وتغييرات الكود اللاحقة في سجل الجلسة نفسه.
...
يحوّل وضع الصور حقل الدردشة إلى إدخال text-to-image مع الاحتفاظ بسجل الجلسة نفسه. استخدمه لإنشاء أصول واجهة أو أيقونات أو ملصقات أو مراجع تصميم قبل الرجوع إلى البرمجة.

1. افتح **Settings** -> **Images**، اختر مزود الصور الافتراضي، ثم أدخل API key أو Account ID أو Base URL أو endpoint محلي لـ ComfyUI حسب متطلبات المزود.
2. في جلسة دردشة، اكتب `/image-mode-start`. يمكنك أيضًا إضافة أول prompt في الرسالة نفسها:

```text
/image-mode-start أيقونة تطبيق نظيفة لوكيل برمجة محلي، تأثير زجاجي، 1024x1024
```

3. أثناء تفعيل الوضع، تولد الرسائل العادية صورًا بدل تنفيذ تعديلات كود. يتحول محدد **Channel** إلى مزودي الصور.
